...
# Script to retrieve data for projects at a central location, e.g. a Jenkins CI
# instance which can access (most) sources, based on environment variables.
# 
...
if [ -z "$listOfProjects" ]; then
	echo "Usage: listOfProjects=... importerTasks=... cleanupRepos=... \\"
	echo "skipGather=... IMPORTER_BASE=... relPath=... restoreFiles=... \\"
	echo "SKIP_REQUIREMENTS=... ./jenkins.sh"
	exit
fi
...
do
	...
	if [ $skipGather = "false" ]; then
		...
		export_handler history_to_json.py $project --log $logLevel
